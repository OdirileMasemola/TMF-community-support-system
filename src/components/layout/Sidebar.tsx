...
export function Sidebar() {
  ...
  return (
    <aside className="hidden min-h-screen w-72 border-r border-border bg-background p-5 lg:block">
      <div className="mb-8">
        <p className="text-xs font-semibold uppercase tracking-wide text-primary">Themba Molefe Foundation</p>
        <h1 className="mt-1 text-xl font-bold text-foreground">Community Support</h1>
      </div>
        ...
        {visibleItems.map((item, index) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted",
